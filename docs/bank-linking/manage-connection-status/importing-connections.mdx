...
If you already have connections with a service provider — for example, because you previously integrated directly with Plaid or Finicity — you can bring them into Meld. Importing a connection does not involve the customer; there is no widget flow and no re-authentication required. This page is for developers migrating from a direct provider integration to Meld.

## Before you begin

<Info>
- You have the provider-level identifiers for each connection you want to import — a Plaid `accessToken`, or a Finicity `customerId` and `institutionLoginId`
...
<Note>
**Currently, importing connections is supported for Finicity and Plaid.** For other providers, the customer must complete the standard widget flow.
</Note>
...
| :--------------------------------------------------- | :---------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------ |
| `400 Bad Request` — provider not supported           | Importing only works for Finicity and Plaid                                   | Use the standard widget flow for other providers.                                                                                                  |
