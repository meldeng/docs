...
* **Full-screen layout** - Enable `fullScreen` on the account's `CRYPTO` preference so every embed uses the full-viewport layout without passing `fullScreen=true` on each URL. Contact Meld support to configure. Override per embed with `fullScreen=false` in the query string. See [URL parameters → fullScreen](/docs/stablecoins/meld-checkout-integration/meld-checkout-guide/url-parameters#fullscreen).
