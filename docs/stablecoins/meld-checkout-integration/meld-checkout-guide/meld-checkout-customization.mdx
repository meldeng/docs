...
Every option on this page is a field on your account's `CRYPTO` preference. Read your current values at any time — this is the same call the checkout itself makes at boot:
...
curl -s "https://api-sb.meld.io/accounts/{accountId}/preferences/public" \
  ...
  | jq '.preferences[] | select(.type=="CRYPTO") | .preference'
```

Each option below names the field it maps to, so you can match what support sets against what the API returns.
...
Update your account logo in the top-left corner of the UI. Field: `accountLogoUrl`.
...
Support can also move the logo above the form rather than inline, via `showAccountLogoOnTop`.

### Flow Management

Control which transaction types are available:

* **Disable Buy Flow** (`onrampEnabled: false`) - Show only sell/offramp functionality
* **Disable Sell Flow** (`offrampEnabled: false`) - Show only buy/onramp functionality

When exactly one of the two is disabled, the checkout hides the buy/sell toggle rather than showing a dead option.

### Navigation

* **Hide the back button** (`hideBackButton`) - Remove the back control from the checkout header, for embeds where your own UI owns navigation.

### Quote Management

Customize provider quote display:

* **Quote limits** (`maxQuotes`) - Cap the number of quotes returned to the checkout. Applied when quotes are generated, so the cap holds for every Meld Checkout embed on the account. Defaults to 15.
...
* **Default redirect URL** (`returnUrl`) - Fallback URL used when an embed does not pass its own `redirectUrl`. Applied when the provider session is created, so it covers every Meld Checkout embed on the account.
...
* **Provider prioritization** - Promoting specific providers to the top of the list. The `topServiceProviders` field exists on the preference but the checkout does not apply it: quote order is set by `rampScore` and cannot be overridden per account.
...
Customize the primary **Buy** or **Sell** CTA button color at the bottom of the UI. Field: `buttonColor`.

<Note>
  An unset button color reads back as the sentinel **`#zzzzzz`**, not `null` or an empty string. The checkout treats that value as "use the default color". If you are reading preferences over the API, test for `#zzzzzz` rather than assuming a colour is configured.
</Note>
...
Set the text color inside the CTA button for optimal contrast. Field: `buttonTextColor`.

#### Border Color

Set the color of the checkout's outer border, and the text drawn on it. Fields: `borderColor` and `borderTextColor`. Both default to Meld's palette (`#0c112a` on `#ffffff`).
