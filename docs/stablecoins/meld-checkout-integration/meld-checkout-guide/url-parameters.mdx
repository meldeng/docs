...
* `transactionTypeLocked` - Pass `BUY` or `SELL` (e.g., `transactionTypeLocked=BUY`). Locks the flow to that direction and hides the buy/sell toggle. Any other value is ignored and the toggle stays visible.
