...
description: "Reference for the legacy query-string parameters that prefill the Meld Checkout UI"
---

# Meld Checkout URL Parameters Reference

<Warning>
**Deprecated — prefilling by query string is being phased out.** Build new integrations against the server-minted URL in the [Full Integration Guide](/docs/stablecoins/meld-checkout-integration/meld-checkout-guide), which carries the same values in a `prefill` object and locks them with `locked`.

This page remains for integrations already built on query strings.
</Warning>

<Warning>
**These parameters are ignored unless your account allows them.** The `widgetUrlGenerationEnforced` preference on your account's `CRYPTO` settings controls this, and it is **`true` for every account created after this setting shipped**. While it is `true`, Meld Checkout ignores query-string prefill entirely and only honours values from a server-minted URL — silently, with no error on the page.

Check your account before relying on anything below:
...
`false` means the parameters below apply. `true` means they do not — use the [Full Integration Guide](/docs/stablecoins/meld-checkout-integration/meld-checkout-guide) instead. The check also fails closed: if Meld cannot read the preference, it behaves as though enforcement is on.
</Warning>

Complete technical reference for the Meld Checkout query-string parameters. Use these to prefill information and customize the user experience.

<Note>
**Sandbox uses a different host.** Examples below use the production host `meldcrypto.com`. In sandbox, use the host from your account's `widgetUrl` preference — `https://sb.meldcrypto.com` for most accounts. A sandbox public key against `meldcrypto.com` will not resolve to your account.
</Note>
...
<Note>
`customerId` is applied to quote requests, where [Ramp Intelligence](/docs/stablecoins/white-label-api-integration/whitelabel-api-guide/ramp-intelligence) uses it to score providers for a returning customer. It is not attached to the checkout session that is created afterwards — use `externalCustomerId` to tie the session and its transactions back to your own user record.
</Note>
...
```
&fullScreen=true     # Stretch widget to 100vh, remove drop shadow
&fullScreen=false    # Default boxed card layout (default)
&fullScreen=1        # Same as true
&fullScreen=0        # Same as false
```
...
1. **URL parameter** — `fullScreen` in the query string
2. **Default** — `false`
...
```
meldcrypto.com/?publicKey=your_public_key&destinationCurrencyCodeLocked=ETH&walletAddressLocked=0x742c8f2e0ce07Dd3f7E78A31E5A97D45c50fF2c8&theme=darkMode
```
...
```
meldcrypto.com/?publicKey=your_public_key&fullScreen=true&theme=lightMode
```
...
```
meldcrypto.com/?publicKey=your_public_key&destinationCurrencyCodeLocked=BONK_SOLANA&walletAddressLocked=6chn9n4CLhNdyBpiLLj9ouUUUmEQYBLWfpUMMuHB9K3k&theme=darkMode
```
