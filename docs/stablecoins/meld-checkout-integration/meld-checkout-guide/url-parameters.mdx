...
1. **URL parameter** — `fullScreen` in the query string (highest priority)
2. **Account preference** — `fullScreen` on the account's `CRYPTO` preference (set by Meld support)
3. **Default** — `false`

Pass `fullScreen=false` in the URL to override an account-level `fullScreen` preference for a specific embed.

> **Note:** Invalid or unrecognized values (for example `fullScreen=yes`) are treated as `false`. When the URL parameter is absent, the account preference is used if present.
