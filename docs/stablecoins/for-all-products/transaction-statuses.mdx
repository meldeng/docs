...
| ----------------- | ---------------------------------------------------------------- | --------- | ---------------------------- |
| `PENDING_CREATED` | Transaction created in Meld system, awaiting provider processing | TEMPORARY | Wait for processing          |
...
Only seen on [ramp-swap](/docs/stablecoins/ramp-swap-flow) transactions, where a ramp leg is followed by a token swap.

| Status           | Description                                        | Type      | User Action         |
| ---------------- | -------------------------------------------------- | --------- | ------------------- |
| `ONRAMP_SETTLED` | Ramp leg settled, crypto arrived, swap still pending | TEMPORARY | Wait for the swap   |
| `SWAPPING`       | Swap provider is swapping tokens                    | TEMPORARY | Wait for completion |
...
| `PARTIALLY_REFUNDED` | Part of the transaction amount has been refunded to the customer | TERMINAL | Refund processed                     |
| `VOIDED`    | Terminated early in the payment lifecycle; no funds moved             | TERMINAL | User can start new transaction       |

<Note>
The table above is the complete set a current transaction can report, so a `switch` over it needs no catch-all beyond the [legacy statuses](#legacy-statuses) below. `VOIDED` overlaps with `CANCELLED` — treat both as "ended before settlement, nothing moved."
</Note>
