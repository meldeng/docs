...
If you are using your own UI to collect this information, use Meld's [Network Partner endpoints](/api-reference/network-partners) to know which countries, payment methods, fiat currencies, and tokens the offramps you have enabled support. The legacy Service Provider endpoints are deprecated and slated for removal — see the [migration guide](/docs/stablecoins/white-label-api-integration/migration-service-provider-to-network-partner).
...
<Warning>
  On a sell, `cryptoDetails.destinationWalletAddress` is the offramp's deposit address. `sourceWalletAddress` and `sessionWalletAddress` are the user's own wallet, so never send crypto to them. On API versions before `2025-03-04`, read the deposit address from `offrampDestinationWalletAddress` instead (`walletAddress` is the user's own wallet).
</Warning>
...
| **Revolut** | Preferred Flow |

<Note>
  Revolut always uses the preferred flow and does not read `"redirectFlow": true`. Instead, pass a `redirectUrl` in `sessionData` (or configure a default return URL on your account). Revolut then redirects the user back to your UI at the "continue to send" step rather than displaying its deposit address, and you fetch that address from `cryptoDetails.destinationWalletAddress` as in Step 3 above. A Revolut sell session with no redirect URL is rejected rather than falling back to the standard flow.
</Note>
...
Once you call [`/transactions`](/api-reference/crypto/retail-ramp/payments-transactions-get) with a `transactionId` (the `paymentTransactionId` in the webhook above), this is a sample response (API version `2025-03-04` or later):
