...
| Field | Meaning |
| --- | --- |
| `id` | Meld headless order id. Webhooks do not carry it — match them via `paymentTransactionId`, then read `orderId` from `GET /payments/transactions/{paymentTransactionId}` |
| `paymentMethodType` | Fiat payout rail used |
| `walletAddress` | Provider-issued address the user must send **crypto** to |

<Warning>
**`SERVICE_PROVIDER_ERROR` on create.** A `500` reading `CryptoAuthorizedAmount <n> exceeds customer deposit <m> — rate may have moved` means the provider re-priced between your quote and the order. Take a fresh quote and submit the order with the amount from that quote rather than a rounded figure, and mint a new `X-Idempotency-Key` for the retry.
</Warning>
...
An offramp produces this sequence. Correlate every one of them with `paymentTransactionId`. For sell flows, `transactionType` is `CRYPTO_SELL`.

| Event | `paymentTransactionStatus` | Meaning |
| --- | --- | --- |
| `TRANSACTION_CRYPTO_PENDING` | `PENDING` | Meld has seen the customer's crypto and created the transaction |
| `TRANSACTION_CRYPTO_TRANSFERRING` | `SETTLING` | The provider is paying fiat out to the details on the order |
| `TRANSACTION_CRYPTO_COMPLETE` | `SETTLED` | Fiat paid out. This is the terminal success event |
| `TRANSACTION_CRYPTO_FAILED` | `FAILED` | The payout could not be completed |
...
<Warning>
  **The webhook payload does not carry the order id.** To match an event back to the order you created in Step 5, take `paymentTransactionId` from the payload, call `GET /payments/transactions/{paymentTransactionId}`, and read `orderId` from the transaction — that value is the `id` the order returned.

  Store the order id against `paymentTransactionId` on the first event you receive, and you only pay for that lookup once per transaction.
</Warning>

<Note>
  **Headless payloads carry no session fields.** `sessionId` and `externalSessionId` only exist for widget transactions, which are created from a session. A headless order has none, and the payload omits empty fields entirely rather than sending them as `null` — so do not key your handler on their presence.
</Note>
