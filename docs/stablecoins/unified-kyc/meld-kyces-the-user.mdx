...
This guide is for developers who want Meld to host the KYC flow. You'll call Meld's API to launch the Sumsub UI, KYC the user, then route them to an onramp (currently Mercuryo and Topper) that accepts the shared KYC token so they don't need to verify again.
