...
- You get a 1 time token from each onramp you use and add it to your Sumsub dashboard
- The onramp you plan to route to supports Sumsub token sharing (currently Mercuryo and Topper)

<Note>
  For sandbox testing details, see [KYC testing](/docs/stablecoins/sandbox-guide/kyc-testing).
</Note>

**This flow works if you KYC the user via Sumsub and send them to the onramp Mercuryo or Topper.**
...
Get a share token from each onramp you will pass the Sumsub KYC token to (Mercuryo and Topper for example) and add them in your Sumsub dashboard in the Partners tab.
...
You will need to pass the Sumsub KYC token, generated for client id **meld.io\_75082**. This is done in the Sumsub dashboard, as shown below.
...
All of this must be under a KYC level named `network-partner-level2.`You will have to fetch the KYC token of the user from Sumsub.
...
If you executed every step correctly, then when you open either the `widgetUrl` or `serviceProviderWidgetUrl`, then the user goes straight to the Mercuryo payment screen, as seen in this demo video.
