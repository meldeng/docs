  ...
  <Step title="Create a customer">
    ...
    <Note>
      **Sandbox note.** 
      ...
      Your account includes a pre-created customer that is already onboarded with the Virtual Accounts providers. Its `externalCustomerId` is `MELD_EXTERNAL_CUSTOMER_ID`.

      You can skip this customer creation, Sumsub KYC, and Virtual Account questionnaire steps and go straight to the **Create a quote** step by using the pre-created customer instead.
    </Note>
  </Step>
...
- **`kycShareProviders`** triggers the fan-out. You can also share one provider at a time and repeat the call for the other; either works. If a provider was already shared, the `POST` returns `409` — retry as a `PATCH` to re-issue. The `PATCH` takes the same body as the `POST`: `serviceProvider` and `mode` are required alongside `kycShareProviders`, and omitting them returns `400`.
...
To identify a customer in requests, you can either provide **"customerId"** or **"externalCustomerId"**. In case of an external ID, you have an option to use a pre-created customer (send **MELD_EXTERNAL_CUSTOMER_ID** verbatim):
...
curl -X POST "$MELD_BASE_URL/payments/crypto/quote?includeSwap=true&integrationMode=HEADLESS" \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "externalCustomerId": "MELD_EXTERNAL_CUSTOMER_ID",
    "countryCode": "US",
    "sourceCurrencyCode": "USD",
    "destinationCurrencyCode": "USDC",
    "sourceAmount": 100,
...
<Warning>
This endpoint takes the Meld `customerId` only. Unlike `POST /payments/crypto/quote`, it does **not** accept `externalCustomerId` — sending one without `customerId` returns `400 [customerId] must not be blank`. Resolve your external id to a Meld `customerId` first via `GET /accounts/customers?externalCustomerId=...`.
</Warning>
...
<Note>
  If Due returns `403` for terms-not-accepted, re-sync KYC via the `PATCH /accounts/customers/{customerId}/kyc/initiate` call above to obtain a fresh terms URL, rather than retrying the provisioning call blindly.
</Note>
...
| `400` with Due's `err_tos_not_accepted` | The customer has not accepted Due Network's terms. Refresh via `PATCH /accounts/customers/{customerId}/kyc/initiate`. |
