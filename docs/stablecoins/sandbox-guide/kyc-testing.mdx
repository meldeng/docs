...
However, if you would like to test the flow end-to-end in Meld's sandbox, please follow this guide.
...
**Ready to test?** Continue below.

# KYC Flow Testing

## Before you begin

The best KYC provider to test with in sandbox is Sumsub, and the best onramp to test with is Mercuryo (for retail ramps) and Noah (for virtual account providers). Therefore, this page uses Sumsub and a combination of Mercuryo and Noahfor the KYC examples. Talk to your account manager to have Sumsub and one of either Mercuryo or Noah set up for your Meld account.

To make the user pass the KYC on Sumsub, there are 2 options:
...
If you already have a user who has passed KYC on your Sumsub account, you can import it into Meld by sharing the Sumsub applicant token (TOKEN\_SHARE mode). [See Sumsub requirements for importing the applicant](https://docs.sumsub.com/docs/copy-applicant#how-to-enable-copy-applicant).

To import the KYC of the user, call `POST /accounts/customers/{customerId}/kyc/initiate`. You will need to pass the Sumsub KYC token, generated for the **meld.io\_75082** client. The `customerId` is in the path, and your request body will look like this:
...
In both KYC initiation modes (**HOSTED\_URL** and **TOKEN\_IMPORT**), you can optionally specify Virtual Account providers to share the user's KYC with after Sumsub approves submission, via the `kycShareProviders` field. Alternatively, add share providers later by calling `PATCH /accounts/customers/{customerId}/kyc/initiate`.
...
```json
{
  "eventType": "CUSTOMER_KYC_STATUS_CHANGE",
  "eventId": "customerId",
  "timestamp": "2026-02-24T16:36:41.717262Z",
  "payload": {
    ...
    "statusUpdatedAt": "2026-02-24T16:36:41.717262Z"
  }
}
```
