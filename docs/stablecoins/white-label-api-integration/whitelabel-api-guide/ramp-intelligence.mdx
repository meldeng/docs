...
<Note>
  Meld's `rampScore` and `lowKyc` optimization features are designed to steer users toward the provider most likely to complete their transaction, improving end-to-end conversion versus showing quotes unranked.
</Note>
...
<Note>
  Scope: These optimization features apply to buy transactions only and are included automatically in all quote responses. **You must pass in a customerId or externalCustomerId when calling quotes for these features to have maximum effect.**
</Note>
...
Every quote carries these conversion-critical fields inside a `rampIntelligence` object — they are **not** top-level properties of the quote:
...
Read them as `quote.rampIntelligence.rampScore`, and so on. Reading `quote.rampScore` yields `undefined`, which fails quietly: a comparator built on it returns `NaN` and leaves the list unsorted.
...
Indicates whether the transaction can complete without document uploads for new users. `null` means unknown — treat it as "no special treatment" rather than `false`.

### `previouslyUsed` (timestamp)

When this customer has already completed a purchase with this provider, this is the **ISO-8601 timestamp of the most recent one**; otherwise it is `null`.

It is a timestamp, not a boolean — test it for presence (`if (previouslyUsed)`), never `=== true`. Use it to drive "You've used this before" messaging, and note that scoring already folds this history into `rampScore`, so `previouslyUsed` is for the label rather than for re-ranking.
...
- "You've used this before" messaging, driven by a non-null [`previouslyUsed`](#previouslyused-timestamp)

**Code Example**:

```typescript
interface RampIntelligence {
  rampScore: number | null;
  lowKyc: boolean | null;
  previouslyUsed: string | null; // ISO-8601 timestamp, or null
}
...
// rampScore is null when scoring could not run, and nulls sort last.
function sortQuotesByRecommendation(quotes: Quote[]): Quote[] {
  return [...quotes].sort(
    (a, b) => (b.rampIntelligence.rampScore ?? -1) - (a.rampIntelligence.rampScore ?? -1),
  );
}
...
function shouldHighlightForSpeed(quote: Quote): boolean {
  return quote.rampIntelligence.lowKyc === true;
}
```
...
function optimizeQuoteDisplay(quotes: Quote[]) {
  return sortQuotesByRecommendation(quotes).map(quote => {
    const { rampScore, lowKyc, previouslyUsed } = quote.rampIntelligence;
    const score = rampScore ?? 0;

    return {
      ...quote,
      recommendationBadge: getRecommendationBadge(score),
      speedBadge: getLowKycBadge(lowKyc),
      familiarityBadge: previouslyUsed ? "You've used this before" : "",
      isPrimary: score >= 70,
