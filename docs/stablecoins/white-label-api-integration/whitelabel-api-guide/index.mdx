...
If you previously integrated against the legacy `/service-providers/*` discovery endpoints, they are deprecated and slated for removal, and are replaced by the category-scoped `/network-partner/*` family used throughout this guide. Migrate as soon as you can — see the [migration guide](/docs/stablecoins/white-label-api-integration/migration-service-provider-to-network-partner).
...
<Steps>
  ...
  <Step title="Create the widget session">
    ...
    <Note>
      **Always send `sessionData.clientIpAddress`.** Pass the end user's public IP (not your server's). Both IPv4 and IPv6 are accepted. Some ramp providers reject or fail sessions without it because they use it for geo and fraud checks; sending it by default keeps the widest set of providers routable.

      If you don't already track the user's IP, resolve it client-side with a free lookup like [`https://api.ipify.org`](https://api.ipify.org) (or an equivalent paid service) and forward it to your backend before creating the session.
    </Note>
...
<Steps>
  ...
  <Step title="Create the session, launch, and surface the deposit address">
    ...
    Send `sessionData.clientIpAddress` here for the same reason as on-ramp — some providers require the end user's IP for geo and fraud checks (see the on-ramp [note](#on-ramp-flow-buy)).

    Open `serviceProviderWidgetUrl`; the provider collects payout-bank details and runs KYC. After the widget completes, fetch the transaction (or look it up by your `externalSessionId`) and read the **deposit address** from `cryptoDetails.destinationWalletAddress` (`offrampDestinationWalletAddress` on API versions before `2025-03-04`):
    ...
    <Warning>
      **Never use `cryptoDetails.sourceWalletAddress` or `sessionWalletAddress`** (`walletAddress` on versions before `2025-03-04`) — for off-ramp those are the customer's own origin wallet, not the deposit address. Display the deposit address (as a QR code plus a copyable string) and instruct the user to send **exactly** the quoted crypto amount to it. A wrong address or amount means lost funds — show a hard warning.
    </Warning>
  </Step>
</Steps>
...
The response is **wrapped** in `{ "transaction": { ... } }`. The inner object carries status, amounts, and a `cryptoDetails` block. **Wallet fields are direction-dependent**: `destinationWalletAddress` is where the crypto goes (the user's wallet on **on-ramp**, the provider's **deposit address** on **off-ramp**), and `sourceWalletAddress` is where it comes from (the user's wallet on off-ramp). Versions before `2025-03-04` return `walletAddress` and `offrampDestinationWalletAddress` instead — see [Off-ramp flow](#off-ramp-flow-sell) Step 6.
...
```json
{
  "transaction": {
    ...
    "cryptoDetails": {
      ...
      "sessionWalletAddress": "0xfCFAa8059080D01b27ccA2B1fA086df0853397E6",
...
<Note>
  For off-ramp, the same endpoint also carries the deposit address (`cryptoDetails.destinationWalletAddress`) used in the sell flow. See [Off-ramp flow](#off-ramp-flow-sell) Step 6.
