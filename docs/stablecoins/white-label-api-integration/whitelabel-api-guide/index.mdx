...
### What changed from the legacy discovery endpoints

If you previously integrated against `/service-providers/*`, the discovery surface moved to `/network-partner/*`. Every call is now category-scoped, fiat and crypto currency lists merged into one endpoint, and purchase limits moved into routes.

| Legacy endpoint (removed) | New endpoint | Notes |
|---|---|---|
| `GET /service-providers/properties/countries` | `GET /network-partner/supported/countries?category={CATEGORY}` | Now category-scoped. Item shape is `{ countryCode, name, flag }`. US-state handling moves to the `subdivision` query param on routes. |
| `GET /service-providers/properties/defaults/by-country` | `GET /network-partner/defaults/{country}/{category}` | Country + category are path params. Returns a single object (or `null`), not an array. `defaultCurrencyCode` → `currencyCode`, `defaultPaymentMethods` → `paymentMethods`. |
| `GET /service-providers/properties/fiat-currencies` | `GET /network-partner/supported/currencies?category={CATEGORY}&country={c}&type=FIAT` | **Merged** — fiat is now `type=FIAT`. |
| `GET /service-providers/properties/crypto-currencies` | `GET /network-partner/supported/currencies?category={CATEGORY}&country={c}&type=CRYPTO` | **Merged** — same path, switched by `type=CRYPTO`. |
| `GET /service-providers/properties/payment-methods` | `GET /network-partner/supported/payment-methods?category={CATEGORY}&country={c}&currencyCode={f}` | Requires `category` + `country` + `currencyCode`. Send the `method` field downstream, never `name`. |
| `GET /service-providers/limits/fiat-currency-purchases` | `GET /network-partner/supported/routes/{category}/{country}/{source}/{destination}` | **Limits moved into routes** — per-(partner, method) min/max live on `route.paymentMethods[].limits`. Limits are now direction-aware. |
...
app.post('/webhook/meld', async (req, res) => {
  const { transactionId, externalCustomerId } = req.body;
  const tx = await fetchTransaction(transactionId);
  updateTransaction(transactionId, tx);
  notifyUser(externalCustomerId, { transactionId, status: tx.status });
