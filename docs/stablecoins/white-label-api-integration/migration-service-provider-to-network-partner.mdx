...
* `countryName` → `name`, and a `flag` is now included. It is a **CDN URL to an SVG**, not an emoji — render it as an image, not as text.
* `states[]` is gone. A `subdivisions` array replaces it, but it is empty for every country today, so treat it as unpopulated: collect the US state in your own UI and pass it as the `subdivision` query param on [Routes](#5-limits-and-routes).
...
```bash After — crypto
GET /network-partner/supported/currencies?category=CRYPTO_ONRAMP&country=US&type=CRYPTO&network=ETH
```
...
* New fields: `decimalPlaces` (use it for client-side formatting), `symbol` (a CDN image URL for the currency's icon, not a ticker string), `chainId` (the EVM chain id as a string, e.g. `"1"`), and `contract` (token contract address) to disambiguate tokens on the same chain.
* Optional filters: `network`, `partner`, `paymentType`, `providerCurrencyCode`.

<Warning>
**`network` takes the `chainCode` value, and a wrong one fails silently.** Ethereum is `ETH`, not `ETHEREUM` — `network=ETHEREUM` returns **HTTP 200 with an empty `currencies` array**, not an error. Read the codes off `chainCode` in an unfiltered response rather than guessing: today's on-ramp catalogue is led by `ETH`, `SOLANA`, `POLYGON`, `BSC`, and `BASE`. Some match the chain's common name and some do not.
</Warning>
...
[
  ...
  },
  {
    ...
    "logo": {
      "dark": "https://cdn.meld.io/images-paymentmethod/APPLE_PAY/logo_dark.png",
      "light": "https://cdn.meld.io/images-paymentmethod/APPLE_PAY/logo_light.png"
    }
  }
]
```
...
* Each row carries a `logo` object with `dark` and `light` CDN URLs, ready to render beside the method name.
* `providerMethod` is the partner's own name for the rail and its casing is not normalised — mostly upper snake case (`CARD`, `APPLE_PAY`, `ACH_BANK_ACCOUNT`, `FIAT_WALLET`) but not always. Never match on it; match on `method`.
    ...
    "paymentMethods": [
      {
        ...
        "limits": { "currencyCode": "USD", "min": 2.00, "max": 7500.00 },
        "headlessSupported": false,
        "logo": {
          "dark": "https://cdn.meld.io/images-paymentmethod/CREDIT_DEBIT_CARD/logo_dark.png",
          "light": "https://cdn.meld.io/images-paymentmethod/CREDIT_DEBIT_CARD/logo_light.png"
