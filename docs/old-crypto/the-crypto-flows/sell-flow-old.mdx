...
If you would like to use Meld's UI for the entire flow, Meld recommends using the [All in One Wizard](/docs/old-crypto/integration-options-old/crypto-widget) which follows the below standard flow. If you use the wizard you do not need to read any of the information below.

The below information applies only if you are planning to use the Meld [White Label Widget](/docs/old-crypto/integration-options-old/crypto-widget).
...
If you are using your own UI to collect this information, you should use Meld's [service provider endpoints](/api-reference/service-providers) to know which countries / payment methods / fiat currencies / tokens the onramps you have enabled support.
...
1. Get a [crypto quote](/docs/old-crypto/integration-options-old/crypto-widget/crypto-quotes) from Meld for each provider. This involves gathering the parameters for what kind of quote the user wants and passing them in to the [crypto quote ](/api-reference/crypto/retail-ramp/crypto-quote-get)endpoint. Meld recommends sorting by the highest `destinationAmount` (which is the amount of fiat the user will receive).
2. The user will select an offramp based on the quotes (or you can select one for them) and launch the onramp's [widget](/docs/old-crypto/integration-options-old/crypto-widget). To trigger the preferred flow, make sure you pass in `"redirectFlow": true` in the body of the request (within `sessionData`). The user completes KYC and confirms the quote in the widget, however they don't complete the transaction there. Instead they are redirected back to your UI.
...
5. Wait and listen for webhooks, and use them as triggers to fetch transaction data from the [/transactions endpoint](/api-reference/crypto/retail-ramp/payments-transactions-get). For more information on how to do this see [here](/docs/old-crypto/integration-options-old/crypto-widget/fetch-transaction-information). Once you have transaction data, you can choose how you would like to display it to the user.
...
Here is an example of a call to [/quotes](/api-reference/crypto/retail-ramp/crypto-quote-get) when selling cryptocurrency (note that passing in serviceProviders is optional here):
...
Here is an example of the corresponding call to [/widget](/api-reference/crypto/retail-ramp/crypto-session-widget-create) when selling cryptocurrency. `walletAddress` is optional on a sell. If you pass it, use the user's own wallet (the one they send crypto from), never a deposit address.
...
Once you call the [/transactions](/api-reference/crypto/retail-ramp/payments-transactions-get) with a transactionId (the `paymentTransactionId` in the webhook above), this is a sample response (API version `2025-03-04` or later):
...
```json
{
    ...
    "paymentDetails": null,
    "cryptoDetails": {
        "sourceWalletAddress": "0xfCFAa8059080D01b27ccA2B1fA086df0853397E6",
        "destinationWalletAddress": "0x9c4e1f2a7b3d5e6f8a0b1c2d3e4f5a6b7c8d9e0f",
