...
If you would like to use Meld's UI for the entire flow, Meld recommends using the [All in One Wizard](/docs/old-crypto/integration-options-old/crypto-widget) which follows the above flow. If you use the wizard you do not need to read any of the information below.
...
Here is an example of a call to [/quotes](/api-reference/crypto/retail-ramp/crypto-quote-get) when buying cryptocurrency (note that passing in serviceProviders is optional here):
...
Note that the customer scores returned are Meld's recommendation of onramp with which the user is most likely to succeed in buying crypto, with a higher customer score being better. Learn more about customerScore [here](/docs/old-crypto/integration-options-old/crypto-widget/crypto-quotes#ranking-quotes). 
...
Here is an example of the corresponding call to [/widget](/api-reference/crypto/retail-ramp/crypto-session-widget-create) when buying cryptocurrency:
