...
Meld's All in One Wizard does not support NFT Checkouts. Instead, please use [Meld's White Label Widget](/docs/old-crypto/integration-options-old/crypto-widget) instead.
...
You should use Meld's [service provider endpoints](/api-reference/service-providers) to know which countries / payment methods / fiat currencies / tokens the onramps you have enabled support.

## Steps to User Buying an NFT

Once you have the above information, you can complete the following steps, in order:

1. You will choose an onramp and launch the onramp's [widget](/docs/old-crypto/integration-options-old/crypto-widget). In this case the user isn't choosing the onramp because a particular NFT will typically only be available on a particular onramp, unlike crypto tokens which are found on multiple onramps. The user completes the transaction using the widget. This entails the user paying for the NFT and the onramp sending the NFT to the user's wallet address.
2. Get transaction data. For more information on how to do this see [here](/docs/old-crypto/integration-options-old/crypto-widget/fetch-transaction-information).
